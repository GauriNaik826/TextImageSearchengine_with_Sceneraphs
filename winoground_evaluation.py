...
class Winoground_evaluation:
    ...
    def evaluate_open_clip_on_winoground(self):
        ...
        for i in range(400):
            text0 = data[i]["caption_0"]
            text1 = data[i]["caption_1"]
            text0 = tokenizer(text0).to(device)    
            text1 = tokenizer(text1).to(device)   
            first_image = data[i]["image_0"]
            second_image = data[i]["image_1"]
            image0 = preprocess(Image.open('./data/images/images_' + first_image + '.png').convert("RGB")).unsqueeze(0).to(device)
            image1 = preprocess(Image.open('./data/images/images_' + second_image + '.png').convert("RGB")).unsqueeze(0).to(device)
            ...
            winoground_clip_scores.append({"id" : data[i]["id"], "c0_i0": c0_i0, "c0_i1": c0_i1, "c1_i0": c1_i0, "c1_i1": c1_i1})
...
class Winoground_generative_evaluation:
    ...
    """
    def __init__(self, model_name, model, processor=None, tokenizer=None,torch_type=None,device=None,prompt_name=None, evaluation_type=None,no_hard_negatives=None,n_shot=8):
        self.model_name = model_name
        self.model = model
        ...
        with open('./data/examples.jsonl', 'r') as file:
            data = json.load(file)
        random.seed(2023)
        subset_idx = random.sample(range(len(data)), 100)
        fewshot_data = []
        for idx in subset_idx:
            fewshot_data.append(data[idx])
        # len(subset_idx[:20])
        ...
        for captions in captions_pairs.values():
            ...
            for i, (img_file, capts) in enumerate(captions.items()):
                example['image'] = Image.open(f'vlm-compositionality/fewshot/images/{img_file}')
    ...
    def evaluate_winoground(self):
        ...
        random.seed(2023)
        subset_idx = random.sample(range(len(data)), 300)
        # subset_idx = range(len(winoground))
        subset_idx = subset_idx[:10]
        #taking the first 20 for time purposes
        ...
        for idx in tqdm(subset_idx):
            image_0 = Image.open('./data/images/images_' + data[idx]["image_0"] + '.png').convert("RGB")
            # image_1 = winoground[idx]["image_1"].convert("RGB")
            caption_0 = data[idx]["caption_0"]
            print("caption_0", caption_0)
            if self.no_hard_negatives:
                # print("using no Hard negatives")
                if idx+1 < len(data):
                    print("idx+1", idx+1)
                    # image_1 = data[idx+1]["image_1"].convert("RGB")
                    image_1 = Image.open('./data/images/images_' + data[idx+1]["image_1"] + '.png').convert("RGB")

                    caption_1 = data[idx+1]["caption_1"]
                ...
                # image_1 = data[idx]["image_1"].convert("RGB")
                image_1 = Image.open('./data/images/images_' + data[idx]["image_1"] + '.png').convert("RGB")
                caption_1 = data[idx]["caption_1"]
                print("caption_1", caption_1)
